      ...
      <div className="col-12 col-md-6">
        <div className="card h-100">
          <div className="card-body">
                  ...
                  {isPDF(selectedFile) && (
                    <button
                      ...
                      onClick={async () => {
                        ...
                        setConverting(true)
                        try { await convert(selectedFile) } finally { setConverting(false) }
                      }}
                    >
                      Confirm Convert
                    </button>
            ...
            {docId && (
              <p className="text-muted small mt-2 mb-0">doc_id: <code>{docId}</code></p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="col-12 col-md-6">
        <div className="card h-100">
          <div className="card-body">
            ...
            {!jsonData && !converting && (
              <p className="text-muted mb-0">Select a PDF on the left and click “Confirm Convert” to preview structured JSON here.</p>
            )}
            {converting && (
            ...
            {!converting && jsonData && (
              <div>
                <pre className="bg-light p-2 rounded" style={{ maxHeight: 360, overflow: 'auto', whiteSpace: 'pre-wrap' }}>
                ...
                <div className="d-flex gap-2">
                  <a
                    ...
                    download={`${(fileName || 'document').replace(/\.pdf$/i, '')}.json`}
                  >
                    Download JSON
                  </a>
                  <button className="btn btn-success btn-sm" onClick={() => nav('/chunk')}>
                    Continue to Chunk
                  </button>
                </div>
              </div>
